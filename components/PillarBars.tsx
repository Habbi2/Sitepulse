...
export function PillarBars({ scores }: PillarBarsProps) {
  ...
  return (
  <div className="grid gap-3 glass rounded-xl p-4 gradient-border">
      {entries.map(([k,v]) => {
        ...
        return (
          <div key={k} className="flex flex-col gap-1">
            ...
            <div className="h-2 rounded bg-neutral-800 overflow-hidden hb-card">
              <div className={`h-full ${color} transition-all duration-700`} style={{ width: `${Math.min(100, Math.max(0, v))}%` }} />
            </div>
          </div>
        );
      })}
